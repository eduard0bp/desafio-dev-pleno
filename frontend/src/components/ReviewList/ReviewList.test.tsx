...
describe('ReviewList', () => {
  ...
  it('filters the table by status using the pill controls', async () => {
    vi.spyOn(api, 'listReviews').mockResolvedValue([
      makeReview({ company_id: 'Acme', status: 'completed' }),
      makeReview({ company_id: 'Globex', status: 'failed' }),
    ]);
    renderList();
  ...
  it('filters the table by company search', async () => {
    vi.spyOn(api, 'listReviews').mockResolvedValue([
      makeReview({ company_id: 'Acme Corp' }),
      makeReview({ company_id: 'Globex' }),
    ]);
    renderList();
    await waitFor(() => expect(screen.getByText('Acme Corp')).toBeInTheDocument());
  ...
  it('paginates when there are more reviews than fit on one page', async () => {
    const reviews = Array.from({ length: 12 }, (_, i) => makeReview({ id: String(i), company_id: `Empresa ${i}` }));
